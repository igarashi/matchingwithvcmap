...
namespace utils {
	namespace alphabet {
		...
		inline std::vector<int> x_transform(const std::vector<int>& text, const int& p) {
		...
		inline std::vector<int> pow_elements(const std::vector<int>& arr)
		{
			auto result = std::vector<int>(arr.size());
