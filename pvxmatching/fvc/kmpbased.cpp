...
namespace fvc {
		...
		for (int step = 2; step < pattern_length_ + 1; step++) {
			...
			for (auto &prev : borders[step - 1]) {
				...
				if (utils::alphabet::is_variable(subseq_char)) {
					...
					if (!exists.second) {
						if (!current.add_condition(exists.first->second, original_char)) {
							continue;
						}
	...
	void KmpBased::calculate_nextbounds(int position, int index, std::map<int, int> &bounds) const {
	...
	bool KmpBased::match(int current, int &pos, std::map<int, int> &bounds) const {
		...
		while (true) {
			...
			if (mismatch || match) {
				...
				calculate_nextbounds(pos, rightmost, bounds);
